...
export async function POST(request) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (session.metadata?.servicePlan && SERVICE_PLANS[session.metadata.servicePlan]) {
      ...
      try {
        ...
        if (lineItems.data && lineItems.data.length > 0) {
          ...
          if (PRICE_ID_TO_PLAN[priceId]) {
            servicePlan = PRICE_ID_TO_PLAN[priceId].id;
            servicePlanDisplay = PRICE_ID_TO_PLAN[priceId].display;
            console.log("Service plan determined from fetched line items:", servicePlan);
          }
        }
      } catch (error) {
    ...
    const orderDetails = {
      ...
      dayOfPickup: session.metadata?.dayOfPickup ? 
        (DAYS_OF_WEEK[session.metadata.dayOfPickup] || session.metadata.dayOfPickup) : "Not specified",
      timeOfPickup: session.metadata?.timeOfPickup ?
        (TIME_SLOTS[session.metadata.timeOfPickup] || session.metadata.timeOfPickup) : "Not specified",
